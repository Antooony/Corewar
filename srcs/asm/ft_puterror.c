...
/*   Updated: 2017/11/17 17:25:48 by adenis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_puterror(t_token *tok, int i, int a, t_token *temp)
{
	...
	{
		if (tok->dirlab)
			ft_printf("Syntax error at token [TOKEN][%d:%d] DIRECT_LABEL \
				\"%s\"\n", tok->lan, tok->col, tok->dirlab);
		else if (tok->dir)
			ft_printf("Syntax error at token [TOKEN][%d:%d] DIRECT \
				\"%s\"\n", tok->lan, tok->col, tok->dir);
		else if (tok->indirlab)
			ft_printf("Syntax error at token [TOKEN][%d:%d] INDIRECT_LABEL \
				\"%s\"\n", tok->lan, tok->col, tok->indirlab);
		else if (tok->indir)
			ft_printf("Syntax error at token [TOKEN][%d:%d] INDIRECT \
				\"%s\"\n", tok->lan, tok->col, tok->indir);
		else if (tok->reg)
			ft_printf("Syntax error at token [TOKEN][%d:%d] REGISTRE \
				\"%s\"\n", tok->lan, tok->col, tok->reg);
		else if (tok->lab)
			ft_printf("Syntax error at token [TOKEN][%d:%d] LABEL \
				\"%s\"\n", tok->lan, tok->col, tok->lab);
		else if (tok->inst)
			ft_printf("Syntax error at token [TOKEN][%d:%d] INSTRUCTION \
